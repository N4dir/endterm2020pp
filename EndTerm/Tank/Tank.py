...
class Tank:
    def __init__(self, color, color1, Place_x, d_right = pygame.K_RIGHT, d_left = pygame.K_LEFT, d_up = pygame.K_UP, d_down = pygame.K_DOWN):
    ...
    def draw(self):
        ...
        if self.direction == Direction.RIGHT:
            pygame.draw.line(screen, self.color1, 
                             (Centre_tank[0] + 19, Centre_tank[1]), (self.x + self.width + self.width // 2, self.y + self.width // 2), 4)
        if self.direction == Direction.LEFT:
            pygame.draw.line(screen, self.color1, 
                             (Centre_tank[0] - 19, Centre_tank[1]), (self.x - self.width // 2, self.y + self.width // 2), 4)
        if self.direction == Direction.UP:
            pygame.draw.line(screen, self.color1, 
                             (Centre_tank[0], Centre_tank[1] - 20), (self.x + self.width // 2, self.y - self.width // 2), 4)
        if self.direction == Direction.DOWN:
            pygame.draw.line(screen, self.color1, 
                             (Centre_tank[0], Centre_tank[1] + 19), (self.x + self.width // 2, self.y + self.width + self.width // 2), 4)
